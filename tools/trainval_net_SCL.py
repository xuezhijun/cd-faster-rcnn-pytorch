...
if __name__ == '__main__':
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            rois_label, \
            out_d_inst, out_d1, out_d2, out_d3 = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)
            ...
            if step % args.disp_interval == 0:
                ...
                # logger.write("[session %d][epoch %2d][iter %4d/%4d] loss: %.4f, lr: %.2e" % (args.session, epoch, step, iters_per_epoch, loss_temp, lr))
                # logger.write("\n fg/bg=(%d/%d), time cost: %f" % (fg_cnt, bg_cnt, end - start))
                # logger.write("\n rpn_cls: %.4f, rpn_box: %.4f,\n rcnn_cls: %.4f, rcnn_box %.4f" % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box))
                # logger.write("\n dloss s1: %.4f, dloss t1: %.4f" % (dloss_s1, dloss_t1))
                # logger.write("\n dloss s2: %.4f, dloss t2: %.4f" % (dloss_s2, dloss_t2))
                # logger.write("\n dloss s3: %.4f, dloss t3: %.4f" % (dloss_s3, dloss_t3))
                # logger.write("\n dinst_s: %.4f, dinst_t: %.4f" % (dloss_s_p, dloss_t_p))
                # logger.write("\n eta: %.4f \n" % (args.eta))
                # logger.close()
                ...
                print("\t\t\t dloss_s1: %.4f, dloss_t1: %.4f, dloss_s2: %.4f, dloss_t2: %.4f" % (dloss_s1, dloss_t1, dloss_s2, dloss_t2))
                print("\t\t\t dloss_s3: %.4f, dloss_t3: %.4f, dinst_s: %.4f, dinst_t: %.4f" % (dloss_s3, dloss_t3, dloss_s_p, dloss_t_p))
        ...
        save_name = os.path.join(output_dir,
                                 'SCL_{}_gamma_{}_session_{}_epoch_{}_step_{}.pth'.format(
                                     args.dataset_t, args.gamma, args.session, epoch, step))
        save_checkpoint(
            {
