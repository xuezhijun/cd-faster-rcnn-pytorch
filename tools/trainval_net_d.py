...
from model.utils.net_utils import CORAL
...
if __name__ == '__main__':
    ...
    if args.net == 'vgg16':
        fasterRCNN = vgg16(imdb.classes, pretrained=True, class_agnostic=args.class_agnostic, fc=args.fc, cv=args.cv)
    else:
    ...
    lr_decay_step = sorted([int(decay_step) for decay_step in args.lr_decay_steps.split(',') if decay_step.strip()])
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            if args.fc and args.cv:
                rois, cls_prob, bbox_pred, \
                rpn_loss_cls, rpn_loss_box, \
                RCNN_loss_cls, RCNN_loss_bbox, \
                rois_label, \
                out_CoralCV_feat_s, out_CoralFC_feat_s = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)
            elif args.fc:
                rois, cls_prob, bbox_pred, \
                rpn_loss_cls, rpn_loss_box, \
                RCNN_loss_cls, RCNN_loss_bbox, \
                rois_label, \
                out_CoralFC_feat_s = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)
            elif args.cv:
                rois, cls_prob, bbox_pred, \
                rpn_loss_cls, rpn_loss_box, \
                RCNN_loss_cls, RCNN_loss_bbox, \
                rois_label, \
                out_CoralCV_feat_s = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)
            ...
            if args.fc and args.cv:
                out_CoralCV_feat_t, out_CoralFC_feat_t = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, target=True)
            elif args.fc:
                out_CoralFC_feat_t = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, target=True)
            elif args.cv:
                out_CoralCV_feat_t = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, target=True)

            # make dloss and loss the same order of magnitude
            if args.fc and args.cv:
                dloss_cv = CORAL(out_CoralCV_feat_s, out_CoralCV_feat_t) * epoch * 1000
                dloss_fc = CORAL(out_CoralFC_feat_s, out_CoralFC_feat_t) * epoch * 1000
                dloss = dloss_cv + dloss_fc
            elif args.fc:
                dloss = CORAL(out_CoralFC_feat_s, out_CoralFC_feat_t) * epoch * 1000
            elif args.cv:
                dloss = CORAL(out_CoralCV_feat_s, out_CoralCV_feat_t) * epoch * 1000
        ...
        if args.fc and args.cv:
            save_name = os.path.join(output_dir,
                                 'frcnn_d_cv+fc_{}_session_{}_epoch_{}_step_{}.pth'.format(
                                     args.dataset_t, args.session, epoch, step))
        elif args.fc:
            save_name = os.path.join(output_dir,
                                 'frcnn_d_fc_{}_session_{}_epoch_{}_step_{}.pth'.format(
                                     args.dataset_t, args.session, epoch, step))
        elif args.cv:
            save_name = os.path.join(output_dir,
                                 'frcnn_d_cv_{}_session_{}_epoch_{}_step_{}.pth'.format(
                                     args.dataset_t, args.session, epoch, step))
