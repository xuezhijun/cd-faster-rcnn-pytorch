...
from datasets.synthia_cycle import synthia_cycle
...
# Set up cityscape_synthia voc style
for split in ['train', 'val']:
...
# Set up synthia voc style
for split in ['train']:
    name = 'synthia_{}'.format(split)
    __sets[name] = (lambda split=split: synthia(split))
# Set up synthia_cycle voc style
for split in ['train']:
    name = 'synthia_cycle_{}'.format(split)
    __sets[name] = (lambda split=split: synthia_cycle(split))
