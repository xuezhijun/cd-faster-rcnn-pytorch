...
class roibatchLoader(data.Dataset):
    def __init__(self, roidb, ratio_list, ratio_index, batch_size, num_classes, training=True, normalize=None,
                 seg_return=False, path_return=False, icr=False):  ####
    ...
    def __getitem__(self, index):
        ...
        blobs = get_minibatch(minibatch_db, self._num_classes,
                              seg_return=self.seg_return, path_return=self.path_return, icr=self.icr)  ####
        ...
        if self.training:
            ...
            padding_data = padding_data.permute(2, 0, 1).contiguous()  # HWC -> CHW
            im_info = im_info.view(3)
            ########
            if self.icr:
                if self.seg_return:
                    seg_map = torch.from_numpy(np.resize(blobs['seg_map'], (data_height, data_width)))
                    return padding_data, im_info, gt_boxes_padding, num_boxes, seg_map, cls_lb
                elif self.path_return:
                    return padding_data, im_info, gt_boxes_padding, num_boxes, blobs['path'], cls_lb
                else:
                    return padding_data, im_info, gt_boxes_padding, num_boxes, cls_lb
            else:
                if self.seg_return:
                    seg_map = torch.from_numpy(np.resize(blobs['seg_map'], (data_height, data_width)))
            ...
            data = data.permute(0, 3, 1, 2).contiguous().view(3, data_height, data_width)  # NHWC -> NCHW
            im_info = im_info.view(3)
