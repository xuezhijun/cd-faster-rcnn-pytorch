...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, target=False, eta=1.0):  ####
        ...
        dim = list(base_feat.size())[1] * list(base_feat.size())[2] * list(base_feat.size())[3]
        d_img = self.netD_img(grad_reverse(base_feat, lambd=eta))
        ########
        ...
        ########
        d_inst = self.netD_inst(grad_reverse(pooled_feat, lambd=eta))

        if target:
            return d_img, d_inst, dim
        ########
