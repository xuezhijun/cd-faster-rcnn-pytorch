...
def CORAL(source, target):
    d = source.data.shape[1]  # dimension of feat
...
class GCN(nn.Module):
    ...
    def forward(self, x, w, adj):
        ...
        #weight = torch.sigmoid(self.weight(feat_1, adj))
...
def vis_detections(im, class_name, dets, thresh=0.8):
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            ...
            cv2.putText(im, '%s: %.2f' % (class_name, score), (bbox[0], bbox[1] - 6), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), thickness=2)
        #if score > thresh:
